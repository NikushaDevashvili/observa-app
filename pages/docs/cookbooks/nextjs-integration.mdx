...
2. **Server-Side Only**: Initialize Observa on the server (API routes, server components, server actions)
3. **Error Handling**: Always wrap tracking in try-catch
4. **Async Tracking**: Don't block requests on tracking
5. **Conversation IDs**: Use consistent IDs for multi-turn conversations
