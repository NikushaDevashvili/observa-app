...
export async function GET(
  ...
  try {
    ...
    console.log(`[Conversation Messages API] Fetching from: ${url.toString()}`);
    ...
    if (!response.ok) {
      ...
      console.error(`[Conversation Messages API] Backend error (${response.status}):`, errorData);
      return NextResponse.json(
        { 
    ...
    console.error("[Conversation Messages API] Error proxying request:", error);
    console.error("[Conversation Messages API] API_URL:", API_URL);
    return NextResponse.json(
      { 
