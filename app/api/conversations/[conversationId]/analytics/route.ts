...
export async function GET(
  ...
  try {
    ...
    console.log(`[Conversation Analytics API] Fetching from: ${url}`);
    ...
    if (!response.ok) {
      ...
      console.error(`[Conversation Analytics API] Backend error (${response.status}):`, errorData);
      return NextResponse.json(
        { 
    ...
    console.error("[Conversation Analytics API] Error proxying request:", error);
    console.error("[Conversation Analytics API] API_URL:", API_URL);
    return NextResponse.json(
      { 
